...
namespace Genshin_Impact_Mod.Forms.Other
{
    public partial class RandomThings : Form
    {
        ...
        private static async Task<string> GetData(string url)
        {
            ...
            catch (WebException e)
            {
                if (e.Status == WebExceptionStatus.ProtocolError && e.Response is HttpWebResponse response)
                    MessageBox.Show(e.Message, Program.AppName, MessageBoxButtons.OK,
                        (int)response.StatusCode >= 500 ? MessageBoxIcon.Warning : MessageBoxIcon.Information);
                else
        ...
        private async void SkiffyApi(string url)
        {
            ...
            var res = JsonConvert.DeserializeObject<SkiffyBotApi>(json);
        ...
        private async void NekosBest(string url, bool gif)
        {
            ...
            var res = JsonConvert.DeserializeObject<NekosBest>(json);
            ...
            Log.Output(
                $"Received data from nekos.best: {res.Results.FirstOrDefault()?.Anime_name} {res.Results.FirstOrDefault()?.Source_url} {res.Results.FirstOrDefault()?.Url}");
        }
        ...
        private async void PurrBot(string url)
        {
            ...
            var res = JsonConvert.DeserializeObject<PurrBot>(json);
        ...
        {
            var json = await GetData(url);
            var res = JsonConvert.DeserializeObject<NekoBot>(json);

            webView21.CoreWebView2.Navigate(res.Message);

            var color = ColorTranslator.FromHtml(res.Color);
            var rgbColor = Color.FromArgb(Convert.ToInt16(color.R), Convert.ToInt16(color.G), Convert.ToInt16(color.B));
