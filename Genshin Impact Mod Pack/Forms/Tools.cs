...
namespace Genshin_Impact_Mod.Forms
{
    public partial class Tools : Form
    {
        ...
        {
            var success = CreateShortcut();
            if (success)
                MessageBox.Show(@"The shortcut has been successfully created.", Program.AppName, MessageBoxButtons.OK,
        ...
        {
            var gamePath = File.ReadAllText($@"{Program.AppData}\game-path.sfn");
            var gameDir = Directory.GetParent(gamePath)?.FullName;
            var cfgFile = $@"{gameDir}\ReShade.ini";

            if (!Directory.Exists(gameDir) || !File.Exists(cfgFile))
                MessageBox.Show($@"ReShade config file was not found in {cfgFile}.", Program.AppName,
        ...
        {
            await Cmd.Execute("notepad", $@"{Program.Folder}\data\unlocker\unlocker.config.json", null, false, true,
                false);
        }
        ...
        private async void ReShadeLogs_Button(object sender, EventArgs e)
        {
            var gamePath = File.ReadAllText($@"{Program.AppData}\game-path.sfn");
            var gameDir = Directory.GetParent(gamePath)?.FullName;
            var logFile = $@"{gameDir}\ReShade.log";

            if (!Directory.Exists(gameDir) || !File.Exists(logFile))
                MessageBox.Show($@"ReShade log file was not found in {logFile}.", Program.AppName, MessageBoxButtons.OK,
